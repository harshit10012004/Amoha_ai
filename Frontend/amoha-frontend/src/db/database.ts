...
export class CareHubDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      medicines: "id,name,time,status",
      careLogs: "++id,category,timestamp,syncStatus",
      carePlan: "id,title,completed"
    });
    ...
    this.version(2).stores({
      ...
      careLogs: "++id,category,timestamp,syncStatus",
      carePlan: "id,title,completed",
      syncQueue: "++id,dataType,status,createdAt,nextRetryAt"
    });
  }
}
...
export async function seedDatabase() {
  ...
  if ((await db.carePlan.count()) === 0) {
    await db.carePlan.bulkAdd([
